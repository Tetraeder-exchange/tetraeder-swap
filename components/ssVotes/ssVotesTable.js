...
import { Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TableSortLabel, TablePagination, Typography, Slider, Skeleton, Accordion, AccordionSummary, AccordionDetails, Button, DialogTitle, DialogContent, Dialog, InputBase, MenuItem, Select } from '@mui/material';
...
const CustomSlider = styled(Slider)(({ theme, appTheme, disabled }) => {
	...
	const MuiSliderRail = {
		background: appTheme === 'dark' ? 'linear-gradient(to left, #1B4F20 50%, #631515 50%)' : 'linear-gradient(to left, #A2E3A9 50%, #EC9999 50%)',
	};
...
const sortIcon = (sortDirection) => {
	...
	return (
		<>
			...
			>
				<path d="M5.83325 8.33337L9.99992 12.5L14.1666 8.33337H5.83325Z" fill={appTheme === 'dark' ? '#5F7285' : '#9BC9E4'} />
			</svg>
		</>
	);
};
				...
				{headCells.map((headCell) => (
					<>
						{headCell.isSticky ? (
							...
							>
								<TableSortLabel active={orderBy === headCell.id} direction={orderBy === headCell.id ? order : 'asc'} IconComponent={() => (orderBy === headCell.id ? sortIcon(order) : null)} onClick={createSortHandler(headCell.id)}>
									<Typography
									...
									.map((row, index) => {
										...
										return (
											<TableRow key={row?.gauge?.address}>
												...
												>
													{tableCellContent(`${numeral(BigNumber(row?.tvl).toLocaleString()).format('($ 0a)')} `, null, null, null)}
												</TableCell>
												<TableCell
													className={classes.cell}
													align="right"
												...
												>
													{tableCellContent(`${formatCurrency(BigNumber(row?.gauge?.apr), 0)}%`, `${formatCurrency(BigNumber(row?.gauge?.expectAPR), 0)}%`, 'Current', 'Next week')}
												</TableCell>
												...
												>
													{tableCellContent(formatCurrency(BigNumber(row?.gauge?.balance).div(row?.gauge?.totalSupply).times(row?.gauge?.reserve0)), formatCurrency(BigNumber(row?.gauge?.balance).div(row?.gauge?.totalSupply).times(row?.gauge?.reserve1)), row.token0.symbol, row.token1.symbol)}
												</TableCell>
												...
												>
													{tableCellContent(formatCurrency(BigNumber(row?.reserve0)), formatCurrency(BigNumber(row?.reserve1)), row.token0.symbol, row.token1.symbol)}
												</TableCell>
												...
												>
													{tableCellContent(formatCurrency(row?.gauge?.weight), `${formatCurrency(row?.gauge?.weightPercent)} %`, null, null)}
												</TableCell>
												...
												>
													{row?.gaugebribes.bribeTokens.length
														? row?.gaugebribes.bribeTokens
																...
																.map((bribe, idx) => {
																	return <>{tableCellContent(formatCurrency(bribe.left), null, bribe.token.symbol + ` (${Number(bribe.apr).toFixed(1)}% APR)`, null)}</>;
																})
														: null}
												</TableCell>
												...
												>
													...
													>
														<div>{tableCellContent(formatCurrency(BigNumber(sliderValue).div(100).times(token?.lockValue)), `${formatCurrency(sliderValue)} %`, null, null)}</div>
														<div>
															<Button
																variant="outlined"
																color="primary"
															...
															<div className={classes.voteTooltip} style={{ display: voteTooltipOpen == row.address ? 'flex' : 'none' }}>
																...
																<div className={classes.voteTooltipBody}>
																	<div className={classes.voteTooltipText}>Move slider or edit your vote manually</div>
										...
										>
											<div>
												...
												>
															...
															{...{
																...
																renderValue:
																	token === null
																		? (selected) => {
																				if (selected === null) {
																					return <div className={cssTokenSelect.placeholder}>Select veCONE NFT</div>;
																				}
																		  }
																		: undefined,
															}}
														...
														>
															{(!vestNFTs || !vestNFTs.length) && (
																<div className={cssTokenSelect.noNFT}>
																	<div className={cssTokenSelect.noNFTtext}>You receive NFT by creating a Lock of your CONE for some time, the more CONE you lock and for the longest time, the more Voting Power your NFT will have.</div>
													...
													<div className={classes.voteTooltipTextModal}>Move slider or edit your vote manually</div>
																...
																>
																	...
																	>
																		...
																		>
																			...
																			{headCell.id === 'apr' && `${formatCurrency(BigNumber.sum(BigNumber(row?.gauge?.apr).div(100).times(40), BigNumber(row?.gauge?.boostedApr0), BigNumber(row?.gauge?.boostedApr1)), 0)}→${formatCurrency(BigNumber.sum(BigNumber(row?.gauge?.apr), BigNumber(row?.gauge?.boostedApr0), BigNumber(row?.gauge?.boostedApr1)), 0)} %`}
																			...
																			{headCell.id === 'apy' && row?.gaugebribes.bribeTokens.length
																				? row?.gaugebribes.bribeTokens
																						.filter((x) => !BigNumber(x?.left).isZero())
																						.map((bribe, idx) => {
																							return <div className={['g-flex-column', 'g-flex--align-end'].join(' ')}>{`${Number(bribe.apr).toFixed(1)}% APR`}</div>;
																						})
																				: null}
																		...
																		>
																			...
																			{headCell.id === 'apy' && row?.gaugebribes.bribeTokens.length
																				? row?.gaugebribes.bribeTokens
																						.filter((x) => !BigNumber(x?.left).isZero())
																						.map((bribe, idx) => {
																							return <div className={['g-flex-column', 'g-flex--align-end'].join(' ')}>{formatSymbol(bribe.token.symbol)}</div>;
																						})
																				: null}
